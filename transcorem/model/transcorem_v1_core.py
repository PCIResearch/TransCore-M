...
from .multimodal_encoder.builder import build_highres_vision_tower
from .multimodal_projector.builder import build_vision_projector

from transcorem.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN


class TransCoreMMetaModel:

    def __init__(self, config):
        super(TransCoreMMetaModel, self).__init__(config)

        if hasattr(config, "mm_vision_tower"):
            self.vision_tower = build_highres_vision_tower(config, delay_load=True)
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        if self.get_vision_tower() is None:
            vision_tower = build_highres_vision_tower(model_args)
...
class TransCoreMMetaForCausalLM(ABC):
        ...
        if vision_tower is None or images is None or input_ids.shape[1] == 1:
            if past_key_values is not None and vision_tower is not None and images is not None and input_ids.shape[1] == 1:
                attention_mask = torch.ones(
                    (attention_mask.shape[0], past_key_values[-1][-1].shape[-3] + 1), dtype=attention_mask.dtype, device=attention_mask.device)
